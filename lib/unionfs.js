(function () {
  ...
  unionfs.prototype.start = function (cb) {
    ...
    tmpa = [
      'sudo mount -o ro --bind ' + this.path_base + ' ' + this.tmp_base_os,
      'sudo mount -o ro --bind ' + this.path_app + ' ' + this.tmp_base_app_app,
      'sudo mount -o rw --bind ' + this.path_rw + ' ' + this.tmp_base_rw,
      'sudo unionfs -o ' + unionfs_opts + ' /rw=RW:/app=RO:/os=RO ' + this.target_path
    ];
        ...
        if (resp != true) {
          ...
          lib.is_mounted(this.tmp_base_app_app, function (respb) {
            if (respb != true) {
              ...
              lib.is_mounted(this.tmp_base_os, function (respc) {
                if (respc != true) {
  ...
  unionfs.prototype.stop = function (cb) {
    if (this.mounted == true) {
      var arr = [
        'sudo umount ' + this.target_path,
        'sudo umount ' + this.tmp_base_rw,
        'sudo umount ' + this.tmp_base_app_app,
        'sudo umount ' + this.tmp_base_os
        // 'rm -Rf ' + this.tmp_base
      ];
      lib.exec_array(tmpa, function () {
