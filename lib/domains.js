          ...
          aliasdomains.get(domain, function (err, doc) {
            if (err) {
              if (err.error == 'not_found') {
                aliasdomains.save(domain, {appname: app._id, host: '127.0.0.1', port: app.port}, function (err, resp) {
                  if (err) {
                    res.error(400, '{"status": "failure - something strange happened"}\n');
                  } else {
          ...
          aliasdomains.get(domain, function (err, doc) {
            if (err) {
              ...
              if (doc.appname == appname) {
                  aliasdomains.delete(domain, function (err, resp) {
                    if (err) {
                      res.error(400, '{"status": "failure - something strange happened"}\n');
                    } else {
