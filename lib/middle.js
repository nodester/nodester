  ...
  if (typeof basicauth != 'undefined' && basicauth.length > 0) {
    ...
    db.get(username, function (err, doc) {
      if (err) {
        ...
        res.end('{"status" : "failure", "message": "Invalid username or password. 1"}\n');
      } else {
    ...
    res.writeHead(401, { 'Content-Type': 'application/json' });
    res.write('{"status" : "failure - authentication. 3"}\n');
    res.end();
  }
};
    ...
    db.get(appname, function (err, doc) {
      if (err) {
        ...
        if (doc.username == req.user._id) {
          ...
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end('{"status" : "failure - authentication. 4"}\n');
        }
      }
    });
  } else {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end('{"status" : "failure - authentication. 5"}\n');
  }
};
