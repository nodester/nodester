...
module.exports = {
  ...
  put: function (req, res, next) {
    ...
    if (newpass) {
      // validate passwords
      if (newpass.indexOf(' ') > -1 || newpass.length < 1) {
        res.writeHead(400, {
          'Content-Type': 'application/json'
        });
        res.write('{"status": "failure - invalid password. must be at least 1 character with no spaces"}\n');
        return res.end();
      }
      if (newuser.match(/^[a-z0-9]+$/i) === null){
        res.writeHead(400, {
          'Content-Type': 'application/json'
        });
