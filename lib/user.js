...
module.exports = {
  ...
  put: function(req, res, next) {
    ...
    if (newpass) {
      ...
      exec('sudo ' + config.opt.main_app_dir + '/scripts/update_authkeys.js ' + config.opt.git_home_dir + '/' + user._id + ' "' + rsakey + '"');
          ...
          if (err) {
            if (err.error == 'not_found') {
              if (typeof rsakey == 'undefined') {
                ...
                exec('sudo ' + config.opt.main_app_dir + '/scripts/update_authkeys.js ' + config.opt.git_home_dir + '/' + newuser + ' "' + rsakey + '"');
                exec('sudo ' + config.opt.main_app_dir + '/scripts/create_user_dir.js ' + config.opt.git_home_dir + '/' + newuser);
                db.save(newuser, {
                  password: lib.md5(newpass),
                  email: email
                }, function(err, resp) {
