  ...
  put: function(req, res, next) {
    ...
    if (newpass) {
      ...
      db.get(user._id, function (err, doc) {
        ...
        var auth_keys = path.join(config.opt.git_home_dir, '.ssh', 'authorized_keys');
        path.exists(auth_keys, function(x) { 
            if (x) {
                ...
                stream.write('command="/usr/local/bin/git-shell-enforce-directory ' + config.opt.git_home_dir + '/' + user._id + '",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty ' + rsakey + '\n', 'utf8');
                stream.end();
          ...
          if (err.error == 'not_found') {
            if (typeof rsakey == 'undefined') {
              ...
              res.end();
                ...
                stream.write('command="/usr/local/bin/git-shell-enforce-directory ' + config.opt.git_home_dir + '/' + newuser + '",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty ' + rsakey + '\n', 'utf8');
                stream.end();
