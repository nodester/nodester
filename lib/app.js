  ...
  gitreset: function(req, res, next) {
    ...
    apps.get(appname, function (err, doc) {
        if (err) {
            ...
            var app_user_home = path.join(config.opt.home_dir, config.opt.hosted_apps_subdir, user._id, app.repo_id);
            var cmd = 'sudo rm -rf "' + app_user_home + '"';
            console.log('cmd:', cmd);
                ...
                if (err) {
                    ...
                    lib.update_proxytable_map(function (err) {
                        app_stop(app.repo_id, function() {
                            ...
                            console.log('cmd:', cmd);
                            exec(cmd, function() {
                                var cmd = 'sudo rm -rf "' + app_user_home + '.git"';
                                console.log('cmd:', cmd);
                                exec(cmd, function() {
                                    ...
                                    repos.save(repo_id, {appname: appname, username: user._id}, function (err, resp) {
                                      if (err) {
                                          ...
                                          res.end(JSON.stringify({status: "failure", message: err.error + ' - ' + err.reason}) + '\n');
                                            ...
                                            var gitsetup = spawn(config.opt.app_dir + '/scripts/gitreposetup.sh', [config.opt.app_dir, config.opt.home_dir + '/' + config.opt.hosted_apps_subdir, user._id, repo_id, start, config.opt.userid]);
      ...
      if (err) {
        ...
        apps.get(doc.appname, function (err, app) {
          if (err) {
            ...
            var cmd = 'cd ' + app_home + ' && ulimit -c unlimited -n 65000 -u 100000 -i 1000000 -l 10240 -s 102400 && sudo ' + path.join(config.opt.app_dir, 'scripts', 'launch_chrooted_app.js') + ' "' + doc.username + '/' + repo_id + '/' + doc.appname + '/' + app.start + ':' + app.port + '"';
            console.log(cmd);
