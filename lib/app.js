...
module.exports = {
  ...
  gitreset: function(req, res, next) {
    ...
    apps.get(appname, function(err, doc) {
      if (err) {
        ...
        exec(config.opt.main_app_dir + '/scripts/gitreset.js ' + app_user_home, function() {
          ...
          if (err) {
            ...
            lib.update_proxytable_map(function(err) {
              app_stop(app.repo_id, function() {
                exec('sudo ' + config.opt.main_app_dir + '/scripts/removeapp.js ' + app_user_home + ' ' + app_git_home, function() {
                  res.end(JSON.stringify({
                    "status": "success"
                  }));
                });
  ...
  put: function(req, res, next) {
    ...
    db.get(appname, function(err, appdoc) {
      if (err) {
        ...
        var app_repo = config.opt.git_user + '@' + config.opt.git_host + ':' + path.join(config.opt.git_home_dir, appdoc.username, appdoc.repo_id + '.git');
  ...
  get: function(req, res, next) {
    res.send({
      status: "success",
      port: req.app.port,
      gitrepo: config.opt.git_user + '@' + config.opt.git_host + ':' + path.join(config.opt.git_home_dir, req.app.username, req.app.repo_id + '.git'),
      start: req.app.start,
                    ...
                    if (err) {
                      ...
                      repos.save(repo_id, {
                        ...
                        if (err) {
                          ...
                          var cmd = 'sudo ' + config.opt.main_app_dir + '/scripts/gitreposetup.sh ' + [config.opt.main_app_dir, config.opt.git_home_dir, user._id, repo_id, start, config.opt.userid, config.opt.git_user, config.opt.apps_home_dir].join(' ');
                          ...
                          // var gitsetup = spawn('/usr/bin/env sudo ' + config.opt.main_app_dir + '/scripts/gitreposetup.sh', [config.opt.main_app_dir, config.opt.git_home_dir, user._id, repo_id, start, config.opt.userid, config.opt.git_user]);
                          ...
                          res.send({
                            status: "success",
                            port: appport,
                            gitrepo: config.opt.git_user + '@' + config.opt.git_host + ':' + path.join(config.opt.git_home_dir, user._id, repo_id + '.git'),
                            start: start,
                            running: false,
                            pid: "unknown"
                          });
    ...
    if (err) {
      ...
      apps.get(doc.appname, function(err, app) {
        if (err) {
          ...
          var configData = {
            appdir: config.opt.main_app_dir,
            userid: config.opt.main_app_uid,
          ...
          lib.tear_down_unionfs_chroot(configData.chroot_base, configData.apphome, configData.apprwfolder, configData.appchroot, function(resp) {
            lib.setup_unionfs_chroot(configData.chroot_base, configData.apphome, configData.apprwfolder, configData.appchroot, function(resp) {
              if (resp === true) {
                var cmd = 'cd ' + configData.appchroot + ' && ulimit -c unlimited -n 65000 -u 100000 -i 1000000 -l 10240 -s 102400 && sudo ' + path.join(config.opt.main_app_dir, 'scripts', 'chroot_runner.js');
                console.log(cmd);
