  ...
  gitreset: function(req, res, next) {
    ...
    apps.get(appname, function (err, doc) {
        if (err) {
            ...
            var app_user_home = path.join(config.opt.git_home_dir, user._id, app.repo_id);
            exec(app_dir + '/scripts/gitreset.js ' + app_user_home, function () {
                ...
                if (err) {
                    ...
                    lib.update_proxytable_map(function (err) {
                        app_stop(app.repo_id, function() {
                          exec(app_dir + '/scripts/removeapp.js ' + app_user_home, function () {
  ...
  put: function(req, res, next) {
    ...
    db.get(appname, function (err, appdoc) {
      if (err) {
        ...
        var app_user_home = path.join(config.opt.git_home_dir, appdoc.username);
                                    ...
                                    repos.save(repo_id, {appname: appname, username: user._id}, function (err, resp) {
                                      if (err) {
                                          ...
                                          res.end(JSON.stringify({status: "failure", message: err.error + ' - ' + err.reason}) + '\n');
                                            ...
                                            var gitsetup = spawn('sudo ' + config.opt.app_dir + '/scripts/gitreposetup.sh', [config.opt.app_dir, config.opt.git_home_dir, user._id, repo_id, start, config.opt.userid, config.opt.git_user]);
...
var app_start = function (repo_id, callback) {
    ...
    db.get(repo_id, function (err, doc) {
      if (err) {
        ...
        var user_home = path.join(config.opt.git_home_dir, doc.username);
        ...
        apps.get(doc.appname, function (err, app) {
          if (err) {
            ...
            var configData = {
                ...
                userid: process.getuid(config.opt.userid),
                apphome: app_home,
                start: app.start,
                port: app.port,
                ip: '127.0.0.1',
