      ...
      if (err) {
        ...
        apps.get(doc.appname, function (err, app) {
          if (err) {
            ...
            console.log('Checking: ', configData.app_home);
            if (!path.existsSync(configData.app_home)) {
                //Bad install??
            ...
            lib.setup_unionfs_chroot(configData.chroot_base, configData.apphome, configData.apprwfolder, configData.appchroot, function (resp) {
              if (resp == true) {
                var cmd = 'cd ' + appchroot + ' && ulimit -c unlimited -n 65000 -u 100000 -i 1000000 -l 10240 -s 102400 && sudo ' + path.join(config.opt.app_dir, 'scripts', 'chroot_runner.js');
                console.log(cmd);
