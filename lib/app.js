...
module.exports = {
  logs: function (req, res, next) {
    ...
    var user = req.user;
    var app = req.app;
    if (user.match(/^[a-z0-9]+$/i) === null && app.username.match(/^[a-z0-9]+$/i) === null){
      res.writeHead(400, {
        'Content-Type': 'application/json'
      });
      res.write('{"status": "failure - invalid username. must be alphanumeric"}\n');
      return res.end();
    }
    if (appname.match(/^[a-z0-9]+$/i) === null){
      res.writeHead(400, {
        'Content-Type': 'application/json'
      });
      res.write('{"status": "failure - invalid appname. must be alphanumeric"}\n');
      return res.end();
    }
  ...
  put: function (req, res, next) {
    ...
    if (appname.match(/^[a-z0-9]+$/i) === null || config.opt.blocked_apps.indexOf(appname) > -1){
      res.writeHead(400, {
        'Content-Type': 'application/json'
      });
      res.write('{"status": "failure - invalid appname. must be alphanumeric"}\n');
        ...
        apps.get(doc.appname, function (err, app) {
          if (err) {
            ...
            if (!path.existsSync(configData.apphome)) {
              //Bad install??
              console.log('App directory does not exist: ', configData.apphome);
              callback(false, err);
              return;
            }
