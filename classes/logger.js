...
logger.prototype.setup = function (app_name, cb) {
  ...
  self.db.get(self.app_name + '_position', function (err, doc) {
    if (err) {
      ...
      self.curr_id = parseInt(doc.current_id);
      cb(null);
    }
  });
};
logger.prototype.log = function (message, cb) {
  ...
  if (self.curr_id != null) {
    self.db.save(self.app_name + '_' + self.curr_id, {message: message}, function (err, res) {
      if (err != null) {
        cb(err);
      } else {
        self.curr_id++;
        self.db.save(self.app_name + '_position', {current_id: self.curr_id}, function (err, res) {
          if (err != null) {
            cb(err);
          } else {
            cb(null);
